...
export default class AllTypesResearch extends Component {
  ...
  renderItem = ({ item }) => (
    <View key={item.key} style={styles.articleListCell}>
      <View style={{ flex: 1 }}>
        ...
        >
          <Text style={styles.articleTitle}>{item.title[0]}</Text>
        </TouchableOpacity>
      </View>
  ...
  findPaperResearchOnFirebase = async () => {
    this.setState({ loading: true })
    this.setState({ articles: [] })
    this.setState({ searchStarted: true })
    this.setState({ offset: 0 })
    this.setState({ unavailableOrNotFoundPaper: false })
    ...
    for (let i = 0; i < this.state.firebasePapers.length; i++) {
      let stringToSearchWithNoSpace = this.state.stringToSearch
        ...
        .toUpperCase()
        .trim()
      let articleTitleString = this.state.firebasePapers[i].Título.replace(
        /\s/g,
        ''
      )
        .toUpperCase()
  ...
  doResearch = async () => {
    if (this.state.searchType === strings.paper) {
      this.findPaperResearchOnFirebase()
    ...
    axios
      .get(
        `https://api.crossref.org/works?query.author=${
          this.state.stringToSearch
        }&filter=type:journal-article&rows=200&offset=${this.state.offset}`
      )
      .then(res => {
        ...
        for (let j = 0; j < works.length; j++) {
          let stringToSearchWithNoSpace = works[j]['container-title'][0]
            .replace(/\s/g, '')
            .toUpperCase()
            .trim()
          for (let i = 0; i < this.state.firebasePapers.length; i++) {
            let articleTitleString = this.state.firebasePapers[
              i
            ].Título.replace(/\s/g, '')
              .toUpperCase()
              .trim()
      ...
      .catch(error => {
        Alert.alert(strings.unavailable_articles_with_this_author)
        this.setState({ loading: false })
      })
  }
    ...
    axios
      .get(
        `https://api.crossref.org/journals/${
          this.state.issnToBeSearched
        }/works?filter=type:journal-article&offset=${this.state.offset}`
      )
      .then(res => {
        const works = res.data.message.items
        if (works.length === 0) {
      ...
      <SafeAreaView style={styles.mainContainer}>
          ...
          <Fragment>
            <View style={styles.researchInputContainer}>
              ...
              <View style={{ flex: 3 }}>
                <TextInput
                  ...
                  onChangeText={texto =>
                    this.setState({ stringToSearch: texto })
                  }
                />
              </View>
            </View>
            <View style={styles.researchSelectContainer}>
              ...
              <View style={styles.dropDownIcon}>
                <Icon name={'sort-desc'} size={25} color={'#ffb131'} />
              </View>
            </View>
          </Fragment>
        )}
        ...
        {!this.state.searchStarted && (
          <View style={styles.searchNotStartedContainer}>
            ...
            <Text style={styles.searchNotStartedText}>
              {strings.no_itens_searched_yet}
            </Text>
          </View>
        )}
        ...
        {this.state.articlePDFVisibility &&
          ...
          <SafeAreaView style={{ flex: 1 }}>
            <TouchableOpacity
              onPress={() => this.setState({ articlePDFVisibility: false })}
            >
              <Icon
